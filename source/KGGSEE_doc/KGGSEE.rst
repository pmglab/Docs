...
- ``--out``: Specify the path and prefix name of the output files. 
    ...
    The third output file is the p-value of all variants belonging to a genes (raw ECS results), named ***.gene.var.pvalue.txt.gz**. Their file formats are the same as above.
     ...
     AveragedLog(p) : the negative log10 of the pvalues generated by averaging the p-values of four selective-expression measures (robust-regression z-score, conventional z-score, MAD robust z-score, and ratio of vector-scalar projection).
